...
public:
	Film() {}
	Film(int wid, int hei) :_w(wid), _h(hei) {}
	...
	void setFilm(int w, int h, double vfov, double dis) {
	...
	void setFovAndDis(double fov, double dis, double aspr) {
		...
		std::cout << "film vfov(in degree): " << _vfov * 180 / M_PI << ", distance: " << _nearPlaneDistance << std::endl;
	}
